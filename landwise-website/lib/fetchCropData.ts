...
export async function fetchCropData(years: number[], geometry: ee.Geometry) {
  ...
  for (const year of years) {
    try {
      ...
      const combinedImages: (ee.ImageType | null)[] = await Promise.all(
      ...
      const validImages: ee.ImageType[] = combinedImages.filter(
        (image): image is ee.ImageType => image !== null
      );

      const vegReflectanceImages = await Promise.all(
        validImages.map(async image => fescBand(image) as Promise<ee.ImageType>)
      );
    
      const vegReflectance = ee.ImageCollection.fromImages(vegReflectanceImages);  
    
      // const vegReflectanceImages = await Promise.all(
      //   validImages.map(fescBand)
      // );
...
async function fescBand(image: ee.ImageType) {
