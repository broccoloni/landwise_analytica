...
export async function fetchClimateData(years: number[], geometry: ee.Geometry) {
  const results: Record<number, ClimateData> = {};
  ...
  for (const year of years) {
    console.log("Fetching climate data for", year);
    try {
      ...
      const separatedClimateDict = await evaluateImage(climateValues) as { [key: string]: any };

      const climateDict: { [key: string]: any } = {};
      ...
      results[year] = { weatherData };

    } catch (error) {
      if (error instanceof Error) {
        console.error(`Failed to fetch climate data for year ${year}:`, error.message);
      } else {
