...
export async function GET() {
  try {
    ...
    const productId = process.env.STRIPE_REPORT_PRODUCT_ID;
    if (!productId) {
      return NextResponse.json(
        { error: 'Report Product ID not found in environment variables' },
        { status: 400 }
      );
    }
    ...
    console.log("Retrieved prices and coupons:", prices, coupons);
    return NextResponse.json({ prices, coupons }, { status: 200 });
  } catch (error) {
