...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get bounding box and width and height for conversion to raster data
    const boundingBox = getBoundingBox(points);
    const { width, height } = calculateDimensions(boundingBox);
    ...
    for (const [year, data] of Object.entries(results)) {
      ...
      historicalLandUse[year] = data.AAFCData['landcover'];
