...
export const fetchClimateData = (basePath: string) => {
  const [yearlyClimateSummary, setYearlyClimateSummary] = useState<Record<number, ClimateData>>({});
  ...
  const processYearlyClimateData = (data: WeatherData[]) => {
    const yearlyData: Record<number, ClimateData> = {};
    ...
    Object.keys(dataByYear).forEach((yearString) => {
      ...
      yearlyData[year] = {
        ...
        lastFrost,
        growingSeasonLength,
        cornHeatUnits: calculateCornHeatUnits(dataByYear[year]),
        GDD0: calculateGDD(dataByYear[year], 0),
        GDD5: calculateGDD(dataByYear[year], 5),
        GDD10: calculateGDD(dataByYear[year], 10),
        GDD15: calculateGDD(dataByYear[year], 15),
      };
    });
