...
export const getBandImagesLegendsUnique = (
  ...
  try {
    ...
    for (const bandName in bands) {
      const bandData = getImageLegendUnique(bands[bandName], valuesToNames, scale);
      if (bandData) {
        newBands[bandName] = bandData;
      } else {
        console.warn(`Skipping band ${bandName} due to missing data.`);
      }
    }
    return newBands;
...
export const getImageLegendUnique = (
  ...
  try {
    ...
    uniqueElements.forEach((value) => {
      const name = value ? valuesToNames[value] : null;
      const index = value ? values.indexOf(value) : null;
      if (name && name !== 'NODATA' && name !== 'Cloud' && index && index >= 0) {
...
export const getBandImagesAndStats = (
  ...
  try {
    ...
    for (const bandName in bands) {
      const bandData = getImageAndStats(bands[bandName], scale);
      if (bandData) {
        newBands[bandName] = bandData;
      } else {
        console.warn(`Skipping band ${bandName} due to missing data.`);
...
export const getImageAndStats = (
  ...
  try {
    ...
    var colorScale = chroma.scale(colors).domain([min, max]);
    if (colors === heatColors) {
      colorScale = colorScale.correctLightness();
    }
...
export function dataToStaticColorUrl(
  ...
  data.forEach((value, i) => {
    const colorIndex = value ? values.indexOf(value) : null ;
    if (value === transparentVal || value === null || colorIndex === null || colorIndex < 0 || colorIndex > colors.length) {
      imageData.data[i * 4] = 0;
      imageData.data[i * 4 + 1] = 0;
      imageData.data[i * 4 + 2] = 0;
      imageData.data[i * 4 + 3] = 0;
